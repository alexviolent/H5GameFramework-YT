	...
	__getset(0,__proto,'bgColor',function(){
		...
		if (Render.isWebGL){
			if (value){
				...
				if (!Browser.onMiniGame && !Browser.onQGMiniGame)Stage._wgColor=null;
			}
		}
...
var Input=(function(_super){
	...
	*/
	...
	__proto._focusIn=function(){
		...
		if(!Browser.onMiniGame && !Browser.onBDMiniGame && !Browser.onTTMiniGame && !Browser.onQGMiniGame){
			var temp=this._text;
			this._text=null;
		}
		this.typeset();
	...
	*/
	...
	Input.__init__=function(){
		...
		if (Browser.onMobile){
			var isTrue=false;
			if(Browser.onMiniGame || Browser.onBDMiniGame || Browser.onKGMiniGame || Browser.onTTMiniGame || Browser.onQGMiniGame){
				isTrue=true;
			}
